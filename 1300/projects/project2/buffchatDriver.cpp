...
// Project 2 Part 2 - Problem 1 - buffchatDriver.cpp
...
int main()
{
    ...
    Buffchat testBuffchat;
    assert(testBuffchat.getNumPosts() == 0);
    assert(testBuffchat.getNumUsers() == 0);
    assert(testBuffchat.getPostSize() == 50);
    assert(testBuffchat.getUserSize() == 50);
    Buffchat emptyBuffchat;

    // Test for readPosts
    int num_posts = testBuffchat.readPosts("no_posts.txt"); // checks if there is no file
    assert(num_posts == -1);
    num_posts = testBuffchat.readPosts("posts.txt"); // tests if function can run normally
    assert(num_posts == 50);
    num_posts = testBuffchat.readPosts("posts.txt"); // tests if num_posts_ is full
    assert(num_posts == -2);

    // Test for printPostsByYear
    testBuffchat.printPostsByYear("22"); // Test for year with posts
    testBuffchat.printPostsByYear("29"); // Test for year without posts
    emptyBuffchat.printPostsByYear("22"); // Test for database with no posts

    // Test for readLikes
    int num_users = testBuffchat.readLikes("no_users.txt"); // checks if there is no file
    assert(num_users == -1);
    num_users = testBuffchat.readLikes("users.txt"); // tests if function can run normally
    assert(num_users == 50);
    num_users = testBuffchat.readLikes("users.txt"); // tests if num_posts_ is full
    assert(num_users == -2);

    // Test getLikes
    assert(testBuffchat.getLikes("michelleryan", "ilovechip") == 4); // tests if both post_author and username are valid
    assert(emptyBuffchat.getLikes("michelleryan", "ilovechip") == -2); // tests if either post or user databases are empty
    assert(testBuffchat.getLikes("michaelryan", "ilovechip") == -3); // tests if post_author is invalid
    assert(testBuffchat.getLikes("michelleryan", "ilovechop") == -3); // tests if username is invalid

    // Test findTagUser
    emptyBuffchat.findTagUser("ryan"); // Tests if database is empty
    testBuffchat.findTagUser("cu"); // Tests if ther are users with the tag
    testBuffchat.findTagUser("12345"); // Tests if ther are users without the tag

    // Test for addPost
    Buffchat addPost;
    assert(addPost.addPost("My new post", "Samuel Jackson", "09/10/22")); // Tests if it can add to an empty database
    assert(!testBuffchat.addPost("My new post", "Samuel Jackson", "09/10/22")); // Tests if it can add to a full database
    
    // Test for printPopularPosts
    testBuffchat.printPopularPosts(15,"19");
    testBuffchat.printPopularPosts(0, "4");
    testBuffchat.printPopularPosts(1005, "1");
    testBuffchat.printPopularPosts(10, "22");
    testBuffchat.printPopularPosts(15,"25");

    //Tests for findLeastActive User
    User leastActive = testBuffchat.findLeastActiveUser(); // Tests populated database
    assert(leastActive.getUsername() == "michelleryan");
    leastActive = emptyBuffchat.findLeastActiveUser(); // Tests empty database
    assert(leastActive.getUsername() == "");
    
    // Test for countUniqueLikes
    assert(testBuffchat.countUniqueLikes("michelleryan") == 112);
    assert(testBuffchat.countUniqueLikes ("goldenbuff3") == 64);
    assert(testBuffchat.countUniqueLikes("roboticscu") == 77);
    assert(testBuffchat.countUniqueLikes ("noaTurner1") == -1);
    assert(testBuffchat.countUniqueLikes ("cuhikingclub") == 79);
